...
const renderItem = (item) => {
    if (typeof item === 'string') {//todo 只有文字而已，目前用span代替，应该有更好的方法
        ...
        const {tag, wrapWithInputGroup} = item.__config__;
        let childHtml
        if (tag === "p-form") {
        ...
        if (wrapWithInputGroup) {
            return renderInputGroup(item, childHtml)
        } else {
            return childHtml
        }
...
const renderProps = (item) => {
        ...
        keys.forEach((propsName) => {
            ...
            if (value === undefined || value === null || value === '') {//这些情况都不生成
                ...
                if (typeOfValue === 'boolean') {//针对boolean，是true直接写一个名称
                    const r = config && config.default;//需要反着写 比如 p-link 中的underline
                    if (r) {
                        if (!value) {
    ...
    if (item.__vModel__) {
        if (formModelName) {
            str.push(` v-model="${formModelName}.${item.__vModel__}"`)//todo p-upload要用v-model:file-list
        } else {
            str.push(` v-model="${item.__vModel__}"`)//todo p-upload要用v-model:file-list
        }
    }
...
const renderInputGroup = (item, child) => {
    const {label,showLabel} = item.__config__
    let str = `<p-input-group>${showLabel?`<p-input-group-addon>${label}</p-input-group-addon>`:''}${child}</p-input-group>`
    return str
}
